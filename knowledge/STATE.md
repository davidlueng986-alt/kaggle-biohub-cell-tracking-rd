...
Last updated: 2026-09-09 (PM GOLD: EXP-0009 splits by sample → per-embryo levels next). Maintainer: gold-watch.
...
- `knowledge/`: this file + HYPOTHESES.md (all five active) + RESULTS.md (EXP-0000…0009 rows).
...
- Next: EXP-0010 per-embryo levels combo (44b6@99.0 + 6bba@98.5) + 6bba@98.0 window probe (H-002); first serious submit only after local gates + timing headroom.
...
# Loop (EXP-0001…0009 done — read them, don't recreate):
bash scripts/run_loop.sh --dry-run
python3 scripts/score.py --dry-run
python3 scripts/test_score.py
# New experiment (EXP-0010 per-embryo combo, H-002):
bash scripts/new_experiment.sh EXP-0010 "<title>"
```
