#!/usr/bin/env python3
"""EXP-0055 scorer driver (lives inside EXP dir; scripts/* untouched on disk).

Same trusted-scorer performance situation as EXP-0039: scripts/score.py
match_nodes uses a pure-Python O(N^3) Hungarian with N = max(n_pred, n_gt)
per frame. At @95.5 (denser than @96.0's ~420 det/frame) that is far too
slow for 100 frames on CPU.
...
def equivalence_gate():
    """Prove scipy solver == pure-python solver on real @95.5 frames."""
...
def main():
    ...
    # ---- @95.5 diagnostics (recall/det/timing) + trusted score
    ...
    print(f"@95.5: det_total={det_total} det/f={det_total/100:.2f} "
    ...
    out = {
        "p955": {"recall": recall, "det_f": det_total / 100,
