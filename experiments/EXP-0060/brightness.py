#!/usr/bin/env python3
"""EXP-0060 step 1: GT-free brightness proxy + SIGN-CORRECTED assignments.

B(S) = mean raw intensity over frames t20-29. Rule (frozen in hypothesis.md):
fit-median split; B > median (HIGH-B) -> pct 96.0 else (LOW-B) 99.0.
Gate always 7.0. LOSO: fit median on other 5. Nested: fit median on embryo A,
apply to B. Recomputed fresh (60 frame reads); cross-checked vs
EXP-0059/brightness.json.
"""
import json
import os
import statistics
...
LOW, HIGH = 96.0, 99.0  # LOW-B -> HIGH pct; HIGH-B -> LOW pct (sign-corrected)
...
def assign(b, med):
    # SIGN-CORRECTED: high frame-mean (bright clutter) -> dense/dim tissue -> 96.
...
def main():
    ...
    ref = json.load(open("/home/box/workspace/kaggle-biohub-rd/experiments/EXP-0059/brightness.json"))["B"]
    for sid in B:
        assert abs(B[sid] - ref[sid]) < 1e-6, f"brightness drift {sid}: {B[sid]} vs {ref[sid]}"
    print("cross-check vs EXP-0059/brightness.json: IDENTICAL (all 6 within 1e-6)", flush=True)
    out = {"B": B, "frames": FRAMES, "low": LOW, "high": HIGH,
           "rule": "HIGH-B -> 96.0 / LOW-B -> 99.0 (sign-corrected vs EXP-0059)",
           "loso": {}, "nested": {}}
    for sid, _ in SAMPLES:
    ...
    for emb in ("44b6", "6bba"):
        ...
        print(f"nested fit={emb} median={med:.4f} -> held-out {'6bba' if emb == '44b6' else '44b6'}: {amap}", flush=True)
