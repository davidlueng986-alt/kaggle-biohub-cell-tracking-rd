...
Last updated: 2026-09-10 (GOLD: EXP-0022 GO — 3.8× timing paydown at parity). Maintainer: gold-watch.
...
- `knowledge/`: this file + HYPOTHESES.md (H-001/H-002/H-004/H-005 active; H-003 classical parked, learned-features residual) + RESULTS.md (EXP-0000…0022 rows).
...
- `experiments/EXP-0022/`: DONE 2026-09-10 — self-calibration + timing (H-002/H-005; `--thr-mode/--downsample` CLI): MAD-pure falsified@probe (background-scale threshold merges everything); half-res DS at parity (rec/raw/ec IDENTICAL, adj up, det −21%, 0.83→0.22 s/f = 3.8×). GO → EXP-0023 full-video DS + kernel upgrade. Detection cost collapses (~22 s/video); linking is next bottleneck candidate.
...
- Next: EXP-0023 full-video DS both samples (H-002/H-005) then kernel upgrade; first serious submit only after local gates + timing headroom.
...
# Loop (EXP-0001…0022 done — read them, don't recreate):
bash scripts/run_loop.sh --dry-run
python3 scripts/score.py --dry-run
python3 scripts/test_score.py
# New experiment (EXP-0023 full-video DS, H-002/H-005):
bash scripts/new_experiment.sh EXP-0023 "<title>"
```
