# Plan — EXP-0056 05db0fb1 @95.0 descent probe

## Config / seeds

- Sample: 6bba_05db0fb1, 100 frames, detector pct=95.0 (fixed a priori).
...
- Scorer: scripts/score.py v1.1.0 (trusted, unmodified on disk); runtime-only
  scipy solver swap inside EXP dir with equivalence gate (EXP-0055 pattern).
- Deterministic, CPU-only. No seeds (detector + linker deterministic).

## Steps

1. `bash experiments/EXP-0056/run.sh` cold end-to-end:
   - detect t=0..99 @95.0 -> full_6bba_05db0fb1_t{t}.json
   - global re-id + BL.link -> 6bba_05db0fb1_pred.json
   - `python3 -u experiments/EXP-0056/score_fast.py` -> scores.json
     (equivalence gate + recall/det diagnostics + trusted score)
   - assemble metrics.json (frozen @95.5 row from EXP-0055, frozen @96.0
     row from EXP-0039 via EXP-0055, frozen @98.5 row from EXP-0021 via
     EXP-0055 — no re-detection) + verdict.
2. Check metrics.json verdict + deltas vs @95.5.
