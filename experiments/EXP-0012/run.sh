#!/usr/bin/env bash
# Runner for EXP-0012 — splitter full-video 6bba@98.0+split.
...
print(f"  {sid}: {len(nodes)} detections ({ns} splits) -> {len(pred['edges'])} links [{pred.get('assign')}]")
EOF
...
echo "[EXP-0012] 2/2: verdict vs @98.5..."
python3 - "$EXP_DIR" <<'EOF'
import json, os, sys
d = sys.argv[1]
...
ref_rec, ref_adj = 0.893, 0.8194  # EXP-0009 @98.5
checks = {"recall_ge_0.95": rec >= 0.95, "adj_ge_985": s["adjusted_edge_jaccard"] >= ref_adj - 1e-12}
metrics = {"exp_id": "EXP-0012", "title": "Peak-splitter full-video 6bba@98.0+split",
           ...
           "decision_reason": ("Splitter verdict per checks. Single deterministic run ceiling holds. "
                               "44b6 untouched (locked @99.0 base).")}
...
print(f"  {sid}@98.0+split: recall={rec:.3f} det/f={row['det_per_frame']:.0f} T_ratio={row['T_ratio']:.2f} "
      f"raw={s['edge_jaccard_raw']:.4f} adj={s['adjusted_edge_jaccard']:.4f} (ref {ref_adj}) "
      f"ec={s['edge_counts']['TP']}/{s['edge_counts']['FP']}/{s['edge_counts']['FN']} t={row['s_per_frame']:.2f}s/f")
print("checks:", checks)
for k, v in checks.items():
    if not v: print("FALSIFIED (recorded):", k)
EOF
echo "[EXP-0012] OK: metrics.json written."
