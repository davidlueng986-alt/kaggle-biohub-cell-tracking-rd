#!/usr/bin/env python3
"""EXP-0039 scorer driver (lives inside EXP dir; scripts/* untouched on disk).

Problem: scripts/score.py match_nodes uses a pure-Python O(N^3) Hungarian
with N = max(n_pred, n_gt) per frame. At @96.0 (~450 det/frame) that is
~1-2 min/frame -> hours for 100 frames (violates <35 min budget).
...
def equivalence_gate():
    ...
    frames = [0, 20, 50, 99]
    for src, tag in (("EXP-0039", "pct96"), ("EXP-0021", "pct98.5")):
        for t in frames:
            ...
            assert abs(c_ref - c_fast) < 1e-6, (src, t, c_ref, c_fast)
            # matched-pair equality (what the scorer actually consumes)
            ...
            assert m_ref == m_fast, (src, t, "pair mismatch")
            print(f"  equiv OK {tag} t={t}: n_pred={n} n_gt={m} "
...
def main():
    ...
    ts = time.time()
    agg98 = SC.score_samples([(SID, pred98, gt, None)])
